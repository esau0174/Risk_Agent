...
from riskflow_agent import AGENT_SCENARIOS, run_agent_workflow
...
def run_cli(
    ...
    workflow_runner: Callable[..., object] = run_agent_workflow,
) -> None:
    _suppress_known_yfinance_future_warning()
        ...
        if tool_name == "calculate_pfe_metrics" and not inserted_credit_loader:
            display_steps.append(
                (
                    ...
                    "Load counterparty exposure profile for Counterparty Risk analysis.",
                )
            )
            inserted_credit_loader = True
...
def _available_input_schemas(detected_modules: list[str]) -> list[str]:
    ...
    if "Credit Risk" in detected_modules or "Counterparty Risk" in detected_modules:
...
def _suppress_known_yfinance_future_warning() -> None:
