...
def _pfe_result() -> dict:
    return {
        ...
        "time_of_peak_pfe_95": 1.0,
        "peak_pfe_99": 2_600_000.0,
        "time_of_peak_pfe_99": 1.0,
        "average_expected_exposure": 1_080_000.0,
        "epe": 1_080_000.0,
...
def test_matching_pfe_commentary_passes_consistency_validation():
    commentary = (
        "Counterparty Exposure / PFE Analysis\n"
        "Peak 95% PFE is 2,100,000.00 at 1.00 years. Average expected exposure "
        "(EPE) is 1,080,000.00. The largest netting set is NS-001. Assumptions and "
        "limitations: the supplied exposure profile is not generated by a full Monte "
        "Carlo pricing engine. This is not investment advice."
    )
...
@pytest.mark.parametrize(
    ("commentary", "expected_error"),
    [
        (
            "Counterparty Exposure / PFE Analysis: Peak 95% PFE is 2,300,000.00 "
            "at 1.00 years. EPE is 1,080,000.00. Assumptions and limitations apply; "
            "not investment advice.",
            "Peak PFE 95 mismatch: expected 2100000.00, found 2300000.00.",
        ),
        (
            "Counterparty Exposure / PFE Analysis: Peak 95% PFE is 2,100,000.00 "
            "at 2.00 years. EPE is 1,080,000.00. Assumptions and limitations apply; "
            "not investment advice.",
            "Time of peak PFE 95 mismatch: expected 1.00 years, found 2.00 years.",
        ),
        (
            "Counterparty Exposure / PFE Analysis: Peak 95% PFE is 2,100,000.00 "
            "at 1.00 years. EPE is 900,000.00. Assumptions and limitations apply; "
            "not investment advice.",
            "EPE mismatch: expected 1080000.00, found 900000.00.",
        ),
    ],
)
def test_mismatched_pfe_commentary_fails(commentary, expected_error):
    result = validate_generated_report(
        None,
        None,
        [],
