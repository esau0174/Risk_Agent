...
def test_exposure_profile_workflow_uses_pfe_tools_and_skips_market_tools(tmp_path):
    ...
    assert result.pfe_result["configured_limit"] is None
    assert result.pfe_result["limit_status"] == "WARNING"
    ...
    assert result.validation_result.warnings == [
        "No credit limit configured for netting set NS-001."
    ]
    pfe_validation_check_names = {
        check.name for check in result.validation_result.checks
    }
    assert "pfe_result_consistency" in pfe_validation_check_names
    assert "credit_limit_utilization" in pfe_validation_check_names
...
def test_exposure_profile_workflow_applies_configured_credit_limits(tmp_path):
    exposure_path = tmp_path / "exposure_profile.csv"
    exposure_path.write_text(
        "netting_set,time_years,expected_exposure,pfe_95,pfe_99\n"
        "NS-001,0.0,100,150,180\n"
    ...
    config_path.write_text(
        json.dumps({"credit_limits": {"NS-001": 275.0}}),
        encoding="utf-8",
    )

    result = run_risk_workflow(
        "Analyze the counterparty exposure profile.",
        data_file=str(exposure_path),
        config_file=str(config_path),
        use_llm=False,
    )

    assert result.pfe_result["configured_limit"] == 275.0
    assert result.pfe_result["limit_utilization"] == pytest.approx(0.8)
    assert result.pfe_result["limit_status"] == "PASSED"
    assert result.validation_result.warnings == []
    assert "Limit utilization for the largest netting set is 80.00%" in (
